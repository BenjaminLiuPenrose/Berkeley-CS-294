...
class QLearner(object):
	...
	def __init__(
		...
		if double_q:
			best_q = tf.reduce_sum(tf.multiply(self.q_tp1, tf.one_hot(self.best_action, self.num_actions)), axis=1)
		else:
			best_q = tf.reduce_max(self.q_tp1, axis=1)

		little_y = self.rew_t_ph + gamma*tf.multiply((1.0-self.done_mask_ph), best_q)
		q_t_taken = tf.reduce_sum(tf.multiply(self.q_t, tf.one_hot(self.act_t_ph, self.num_actions)), axis=1)
		self.total_error = huber_loss(q_t_taken-little_y)
		######END Solution
