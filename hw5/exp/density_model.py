...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            ### BEGIN Solution
            # b is # of data pts; B is # of kernals; ob_dim is dim of the problem/data pts
            # 1. Compute deltas
            deltas = np.array([self.means-state for state in states])
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        # state1, state2: s, s'
        # discriim_target: labels 1/0 of discriminator
        # log_likelihood: log prob(discriminator=discrim_target)
        # likelihood: prob(discriminator=discrim_target)
    ...
    def make_prior(self, z_size):
        ...
        """
        prior_mean = tf.Variable(tf.zeros(z_size), name="prior_mean")
        prior_logstd = tf.Variable(tf.zeros(z_size), name="prior_logstd")
